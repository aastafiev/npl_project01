...
if __name__ == '__main__':
    ...
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO,
                        filename=os.path.join(os.path.dirname(__file__), 'logs', 'Word2Vec_AverageVectors.log'))
    ...
    print "Read %d labeled train reviews, " \
          "%d labeled test reviews, and %d unlabeled reviews\n" % (train["review"].size,
                                                                   test["review"].size,
    ...
    forest = rfc.fit(trainDataVecs, train["sentiment"])

    # Test & extract results
    result = rfc.predict(testDataVecs)
    ...
    logging.info("Wrote Word2Vec_AverageVectors.csv")
